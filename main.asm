...
CLEAR_T1    equ	0x66
CLEAR_T0    equ	0x65
SCORE_INC_H equ	0x64
SCORE_INC_M equ	0x63
TEMP_REG    equ	0x62
...
CHECK_ROWS    
    ...
    CHECK_ROWS_LOOP
	MOVF	CHECK_TEMP,W
	ADDLW	BANK1+B0-1	    ;BECAUSE LOOP ENDS WHEN REG -1 = 0
	MOVWF	FSR
	;CHECK IF ROW NOT ZERO
	MOVLW	0xFF
	XORWF	INDF,W
	BTFSC	STATUS,Z
...
CLEAR_COLLISION
	...
	ADDLW	BANK1+B0	;ADD OFFSET OF BANK
    ...
    MOVLW   BANK1+B0+PLAYABLE_COLS-1
    MOVWF   FSR
    CLRF    INDF
    GOTO    CLEAR_VISUAL
...
CLEAR_VISUAL
    ...
    SUBWF   CLEAR_TEMP,F    ;PLAYABLE_COLS - current position, to loop from down up
    CLEAR_VISUAL_LOOP
    ;POSITION SELF TO NEXT COLUMN
	BCF	STATUS,C
	RLF	CLEAR_INDX,W
	ADDLW	B0
	MOVWF	FSR
    ;RECOVER CURRENT COLUMN
	MOVF	INDF,W
	MOVWF	CLEAR_T0
    ;POSITION SELF TO ACTUAL COLUMN
	MOVLW	-2
	ADDWF	FSR,F
    ;OVERWRITE WITH NEXT COLUMN
	MOVF	CLEAR_T0,W
	CALL	REVERSE_BYTE
	MOVWF	INDF
    ;LOOOOOOOP
...
START
	...
	BTFSS	STATUS,Z
	GOTO	BEND
    ;INCREASE SCORE WHEN BUTTON PRESSED
	MOVLW   START_LEVEL
	MOVWF   TEMP_REG
	MOVF    LEVEL,W
	SUBWF   TEMP_REG,F
	...
	CALL	COLLISION_PIECE
	XORLW	0xFF
	BTFSS	STATUS,Z    ;INVERSED LOGIC, JUST WHEN FAILS CHECKS NO JUMP
	GOTO	LOST_GAME
